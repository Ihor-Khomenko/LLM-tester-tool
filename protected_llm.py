...
FAILED_TESTS_PATH = "protected_failed_tests.json"

def log_failed_test(prompt, reason, response=None):
    failed = []
...
def query_protected(prompt):
    # Input filtering
    input_result = input_scanner.scan(prompt)
    if not input_result["is_safe"]:
        log_failed_test(prompt, f"Input flagged: {input_result['reason']}")
        return f"[LLMGuard blocked input: {input_result['reason']}]"
    # LLM call
    response = ollama.chat(model='llama3', messages=[{'role': 'user', 'content': prompt}])
    content = response['message']['content']
    # Output filtering (fix: use output=content for Toxicity)
    output_result = output_scanner.scan(output=content)
    if not output_result["is_safe"]:
        log_failed_test(prompt, f"Output flagged: {output_result['reason']}", content)
        return f"[LLMGuard blocked output: {output_result['reason']}]"
    return content
...
def main():
    ...
    while True:
        ...
        if "-" in user_input:
            ...
            if parsed is None or not isinstance(parsed[0], int) or not isinstance(parsed[1], int):
    ...
    if selected_data:
        ...
        for i, item in enumerate(selected_data):
            ...
            if not prompt:
                ...
                continue
            try:
                response = query_protected(prompt)
                ...
                failed += 1
        # Log results to file
        log_path = "protected_test_results.json"
        try:
